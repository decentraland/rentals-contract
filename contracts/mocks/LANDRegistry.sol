...
contract IEstateRegistry {
    ...
    event RemoveLand(uint256 indexed _estateId, uint256 indexed _landId, address indexed _destinatary);
    ...
    event SetEstateLandBalanceToken(address indexed _previousEstateLandBalance, address indexed _newEstateLandBalance);
}
...
contract ERC721Base is AssetRegistryStorage, IERC721Base, ERC165 {
    ...
    function _moveToken(
        ...
        bytes userData,
        bool doCheck
    ) private isDestinataryDefined(to) destinataryIsNotHolder(assetId, to) isCurrentOwner(from, assetId) {
        address holder = _holderOf[assetId];
...
interface ILANDRegistry {
    ...
    function ownerOfLandMany(int256[] x, int256[] y) external view returns (address[]);
    ...
    event Update(uint256 indexed assetId, address indexed holder, address indexed operator, string data);
    ...
    event SetLandBalanceToken(address indexed _previousLandBalance, address indexed _newLandBalance);
}
...
contract LANDRegistry is Storage, Ownable, FullAssetRegistry, ILANDRegistry {
    ...
    modifier onlyUpdateAuthorized(uint256 tokenId) {
        require(
            msg.sender == _ownerOf(tokenId) || _isAuthorized(msg.sender, tokenId) || _isUpdateAuthorized(msg.sender, tokenId),
    ...
    modifier canSetUpdateOperator(uint256 tokenId) {
        ...
        require(_isAuthorized(msg.sender, tokenId) || updateManager[owner][msg.sender], "unauthorized user");
        _;
    }
    ...
    function transferLandToEstate(
        ...
        require(estateRegistry.ownerOf(estateId) == msg.sender, "You must own the Estate you want to transfer to");
     ...
     */
    function setLandBalanceToken(address _newLandBalance) external onlyProxyOwner {
    ...
    function registerBalance() external {
        ...
        if (currentBalance > 0) {
            require(landBalance.destroyTokens(msg.sender, currentBalance), "Register Balance::Could not destroy tokens");
        }
        ...
        // Generate Tokens
        require(landBalance.generateTokens(msg.sender, newBalance), "Register Balance::Could not generate tokens");
    }
    ...
    function unregisterBalance() external {
        ...
        // Destroy Tokens
        require(landBalance.destroyTokens(msg.sender, currentBalance), "Unregister Balance::Could not destroy tokens");
    }
